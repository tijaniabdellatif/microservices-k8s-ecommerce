 ...
 const {doRequest,errors,loader} = useRequest({
    ...
    onSuccess : () => {
        ...
        if(!errors){
             ...
             const id = toast.loading('Operation en cours...',{});
             toast.update(id,{render:'logged out',type:'success',isLoading:false,position:'top-center'});
             setTimeout(() =>{
              redirectPage();
             },3000)
...
.map(({id,label,href,icon}) =>{
  ...
  if(href === '/auth/logout'){
   ...
   return (
    ...
    <Link key={id} href={href} className="flex items-center group gap-3">
      {icon}
      <span className="font-quicksand text-sm leading-3 font-medium group-hover:text-light-cta">{label}</span>
      </Link>
